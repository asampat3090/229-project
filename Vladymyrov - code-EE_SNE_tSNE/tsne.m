...
function [X,E,A,T] = tsne(Wp,d,l,opts)
...
for i = 1:length(l)
  ...
  while convcrit
    WpN = Wp.*ker; DpN = diag(sparse(sum(WpN,2)));
    WqN = l(i)*(ker.^2)./sum(ker(:)); DqN=diag(sparse(sum(WqN,2)));
    G = 4*((DpN-WpN)-(DqN-WqN))*Xold;     % gradient
    P = -S*(R\(Rt\(St*G)));               % spectral direction
    [X,e(j+1),ker,a(j+1)] = tsnels(Xold,Wp,l(i),P,e(j),G,a(j)); % line search
  ...
  % save parameters in an array (for single lambda) or in a cell array (for homotopy)
  if(length(l)==1)
    E = e; A = a; T = t;
  else
    E{i} = e; A{i} = a; T{i} = t;
...
% [X,e,ker,alpha] = tsnels(X,Wp,l,P,ff,G[,alpha0,rho,c])
% Backtracking line search for t-SNE
%
...
function [X,e,ker,alpha] = tsnels(X,Wp,l,P,ff,G,alpha0,rho,c)
% ---------- Argument defaults ----------
...
alpha = alpha0;
tmp = c*G(:)'*P(:);
[e,ker] = tsne_error(X+alpha*P,Wp,l);
...
% [e,ker] = tsne_error(X,Wp,l) t-SNE objective function error
function [e,ker] = tsne_error(X,Wp,l)
N = size(Wp, 1);
sqd = sqdist(X); ker = 1./(1+sqd);
ker(1:N+1:end) = 0; ker = max(ker,eps);
e = -Wp(:)'*log(ker(:)) + l*log(sum(ker(:)));
